...
prepare-dmg:
	...
	./create-dmg/create-dmg \
	    ...
	    --window-size 500 320 \
	    --icon-size 80 \
	    --icon "FastPassword.app" 125 175 \
	    --hide-extension "FastPassword.app" \
...
prepare-dSYM:
	echo "Zipping dSYMs..."
	cd $(BUILD_PATH)/FastPassword.xcarchive/dSYMs && zip -r $(PWD)/dSYMs.zip .
...
clean:
	...
	if [ -a $(PWD)/FastPassword.dmg ]; then rm $(PWD)/FastPassword.dmg; fi;

next-version:
	versionNumber=$$(/usr/libexec/PlistBuddy -c "Print CFBundleVersion" "$(PWD)/FastPassword/Supporting Files/Info.plist") ;\
	echo "Actual version is: $$versionNumber" ;\
	versionNumber=$$((versionNumber + 1)) ;\
	echo "Next version is: $$versionNumber" ;\
	/usr/libexec/PlistBuddy -c "Set :CFBundleVersion $$versionNumber" "$(PWD)/FastPassword/Supporting Files/Info.plist" ;\
